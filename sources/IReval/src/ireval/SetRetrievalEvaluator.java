...
public class SetRetrievalEvaluator {
    ...
    public double meanNormalizedDiscountedCumulativeGain1( ) {
        double sumNDCG = 0;
        
        for( RetrievalEvaluator evaluator : _evaluators ) {
            sumNDCG += evaluator.normalizedDiscountedCumulativeGain( 1 );
        }                                                             
        
        return sumNDCG / _evaluators.size();
    }
    
    public double meanNormalizedDiscountedCumulativeGain2( ) {
        double sumNDCG = 0;
        
        for( RetrievalEvaluator evaluator : _evaluators ) {
            sumNDCG += evaluator.normalizedDiscountedCumulativeGain( 2 );
        }                                                             
        
        return sumNDCG / _evaluators.size();
    }
    
    public double meanNormalizedDiscountedCumulativeGain3( ) {
        double sumNDCG = 0;
        
        for( RetrievalEvaluator evaluator : _evaluators ) {
            sumNDCG += evaluator.normalizedDiscountedCumulativeGain(3);
        }                                                             
        
        return sumNDCG / _evaluators.size();
    }
    
    public double meanNormalizedDiscountedCumulativeGain4( ) {
        double sumNDCG = 0;
        
        for( RetrievalEvaluator evaluator : _evaluators ) {
            sumNDCG += evaluator.normalizedDiscountedCumulativeGain(4);
        }                                                             
        
        return sumNDCG / _evaluators.size();
    }
    
    public double meanNormalizedDiscountedCumulativeGain5( ) {
        double sumNDCG = 0;
        
        for( RetrievalEvaluator evaluator : _evaluators ) {
            sumNDCG += evaluator.normalizedDiscountedCumulativeGain( 5);
        }                                                             
        
        return sumNDCG / _evaluators.size();
    }
    
    public double meanNormalizedDiscountedCumulativeGain10( ) {
        double sumNDCG = 0;
        
        for( RetrievalEvaluator evaluator : _evaluators ) {
            sumNDCG += evaluator.normalizedDiscountedCumulativeGain(10);
        }                                                             
        
        return sumNDCG / _evaluators.size();
    }
    
    public double meanNormalizedDiscountedCumulativeGain15( ) {
        double sumNDCG = 0;
        
        for( RetrievalEvaluator evaluator : _evaluators ) {
            sumNDCG += evaluator.normalizedDiscountedCumulativeGain(15);
        }                                                             
        
        return sumNDCG / _evaluators.size();
    }
    
    public double meanNormalizedDiscountedCumulativeGain20( ) {
        double sumNDCG = 0;
        
        for( RetrievalEvaluator evaluator : _evaluators ) {
            sumNDCG += evaluator.normalizedDiscountedCumulativeGain(20);
        }                                                             
